...
struct drawIntegral: Shape {
    ...
    func path(in rect: CGRect) -> Path {
        ...
        for item in drawingPoints {
            ...
            path.addRect(CGRect(x: (item.xPoint-negativexOffset)*Double(scale)/displayRange , y: ((item.yPoint-negativeyOffset)*Double((-1.0/displayRange)*scale)+2.0*Double(center.y)), width: 1.0 , height: 1.0) )
